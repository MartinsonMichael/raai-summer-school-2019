...
class CarRacing(gym.Env, EzPickle):
    ...
    def __init__(self, agent=True, num_bots=1, track_form='X', \
        ...
        low_val = np.array(
            [-PLAYFIELD, -PLAYFIELD, np.finfo(np.float32).min, np.finfo(np.float32).min, -PLAYFIELD, -PLAYFIELD])
        high_val = np.array(
            [PLAYFIELD, PLAYFIELD, np.finfo(np.float32).max, np.finfo(np.float32).max, PLAYFIELD, PLAYFIELD])
